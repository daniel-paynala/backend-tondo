-- ============================================================================
-- 030_tonji_categories_marchands.sql
...
--
-- ⚠️ PROD (`tonji_`). En TEST, jouer `TEST_030_tondo_categories_marchands.sql`.
-- IDEMPOTENT. À jouer dans le SQL Editor Supabase.
-- ============================================================================


-- ----------------------------------------------------------------------------
-- 1. tonji_categories_marchands
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.tonji_categories_marchands (
...
COMMENT ON TABLE public.tonji_categories_marchands IS
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'tonji_categories_marchands_libelle_check') THEN
    ALTER TABLE public.tonji_categories_marchands ADD CONSTRAINT tonji_categories_marchands_libelle_check
      CHECK (length(btrim(libelle)) >= 2);
  END IF;
END $$;

-- Unicité insensible à la casse : c'est précisément le doublon qu'on veut éviter.
CREATE UNIQUE INDEX IF NOT EXISTS tonji_categories_marchands_libelle_uidx
  ON public.tonji_categories_marchands (project_id, lower(libelle));

DROP TRIGGER IF EXISTS trg_tonji_categories_marchands_updated_at ON public.tonji_categories_marchands;
CREATE TRIGGER trg_tonji_categories_marchands_updated_at
  BEFORE UPDATE ON public.tonji_categories_marchands
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

ALTER TABLE public.tonji_categories_marchands ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "tonji_categories_marchands_same_project" ON public.tonji_categories_marchands;
CREATE POLICY "tonji_categories_marchands_same_project" ON public.tonji_categories_marchands
  FOR ALL USING (project_id = public.current_project_id())
  WITH CHECK (project_id = public.current_project_id());


-- ----------------------------------------------------------------------------
-- 2. tonji_marchands.categorie_id
-- ----------------------------------------------------------------------------
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'tonji_marchands_categorie_fk') THEN
    ALTER TABLE public.tonji_marchands ADD CONSTRAINT tonji_marchands_categorie_fk
      FOREIGN KEY (categorie_id) REFERENCES public.tonji_categories_marchands(id) ON DELETE RESTRICT;
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS tonji_marchands_categorie_idx
  ON public.tonji_marchands (categorie_id)
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'tonji_marchands' AND column_name = 'categorie'
  ) THEN
    -- Une catégorie par valeur distincte, projet par projet.
    INSERT INTO public.tonji_categories_marchands (project_id, libelle)
    SELECT DISTINCT m.project_id, btrim(m.categorie)
      FROM public.tonji_marchands m
     WHERE m.categorie IS NOT NULL AND btrim(m.categorie) <> ''
    ON CONFLICT DO NOTHING;

    UPDATE public.tonji_marchands m
       SET categorie_id = c.id
      FROM public.tonji_categories_marchands c
     WHERE c.project_id = m.project_id
       AND lower(c.libelle) = lower(btrim(m.categorie))
       AND m.categorie_id IS NULL;

    ALTER TABLE public.tonji_marchands DROP COLUMN categorie;
  END IF;
END $$;
...
SELECT p.id, v.libelle
 ...
 WHERE NOT EXISTS (
   SELECT 1 FROM public.tonji_categories_marchands c WHERE c.project_id = p.id
 )
ON CONFLICT DO NOTHING;
...
-- ----------------------------------------------------------------------------
GRANT ALL ON public.tonji_categories_marchands TO service_role;


-- ============================================================================
-- FIN 030_tonji_categories_marchands.sql
-- ============================================================================
