...
BEGIN
  ...
  -- Retraits
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'tonji_retraits_especes_pkey') THEN
    ALTER TABLE public.tonji_retraits_especes ADD CONSTRAINT tonji_retraits_especes_pkey PRIMARY KEY (id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'tonji_retraits_especes_project_fk') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'tonji_retraits_especes_cagnotte_fk') THEN
...
DROP TRIGGER IF EXISTS trg_tonji_retraits_especes_updated_at ON public.tonji_retraits_especes;
CREATE TRIGGER trg_tonji_retraits_especes_updated_at
  BEFORE UPDATE ON public.tonji_retraits_especes
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
ALTER TABLE public.tonji_retraits_especes    ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS "tonji_retraits_especes_same_project" ON public.tonji_retraits_especes;
CREATE POLICY "tonji_retraits_especes_same_project" ON public.tonji_retraits_especes
  FOR ALL USING (project_id = public.current_project_id())
  WITH CHECK (project_id = public.current_project_id());
...
GRANT ALL ON public.tonji_retraits_especes    TO service_role;
