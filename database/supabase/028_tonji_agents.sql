...
--
-- ⚠️ PROD (`tonji_`). En DEV/TEST, remplacer par `tondo_`. IDEMPOTENT.
-- ============================================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'tonji_agents_pkey' AND conrelid = 'public.tonji_agents'::regclass
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'tonji_agents_project_fk' AND conrelid = 'public.tonji_agents'::regclass
  ) THEN
    ALTER TABLE public.tonji_agents
      ADD CONSTRAINT tonji_agents_project_fk
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'tonji_agents_type_check'
  ) THEN
    ALTER TABLE public.tonji_agents
      ADD CONSTRAINT tonji_agents_type_check
      CHECK (type IN ('tpe', 'guichet'));
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'tonji_agents_statut_check'
  ) THEN
    ALTER TABLE public.tonji_agents
      ADD CONSTRAINT tonji_agents_statut_check
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'tonji_agents_code_format_check'
  ) THEN
    ALTER TABLE public.tonji_agents
      ADD CONSTRAINT tonji_agents_code_format_check
  ...
  -- son statut le dise.
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'tonji_agents_plafond_check'
  ) THEN
    ALTER TABLE public.tonji_agents
      ADD CONSTRAINT tonji_agents_plafond_check
...
CREATE UNIQUE INDEX IF NOT EXISTS tonji_agents_code_uidx
  ON public.tonji_agents (project_id, code);
...
CREATE UNIQUE INDEX IF NOT EXISTS tonji_agents_cle_uidx
  ON public.tonji_agents (cle_api_hash)
...
--
--    ALTER TABLE public.tonji_agents DROP CONSTRAINT tonji_agents_type_check;
--    ALTER TABLE public.tonji_agents
--      ADD CONSTRAINT tonji_agents_type_check
